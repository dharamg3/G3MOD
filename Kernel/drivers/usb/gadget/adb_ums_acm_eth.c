...
#define RNDIS_ACM_UMS_ADB_PRODUCT_ID 		0x687b	
...
static struct usb_device_descriptor device_desc = {
	...
	.idProduct            = __constant_cpu_to_le16(UMS_PRODUCT_ID),
...
static int rndis_ums_adb_acm_bind_config(struct usb_configuration *c)
{
	struct android_dev *dev = _android_dev;
	int ret;
...
#define	ANDROID_DEBUG_CONFIG_STRING "RNDIS + ADB + UMS + ACM (Debugging mode)"
...
static struct usb_configuration rndis_ums_adb_acm_config  = {
	.label		= ANDROID_DEBUG_CONFIG_STRING,
	.bind		= rndis_ums_adb_acm_bind_config,
...
//	printk(KERN_INFO "android_bind\n");
	...
	printk("[RNDIS_ADB_UMS_ACM] string_dev = %s \n",strings_dev[STRING_SERIAL_IDX].s);
...
static void enable_adb(struct android_dev *dev, int enable)
{
	...
	if (enable != dev->adb_enabled) {
		...
		if (enable)
		{			
			ret = usb_change_config(dev->cdev, &rndis_ums_adb_acm_config);
			if (ret) {
...
static int __init init(void)
{
	...
	dev->product_id = UMS_PRODUCT_ID;
	dev->adb_product_id = RNDIS_ACM_UMS_ADB_PRODUCT_ID;
	_android_dev = dev;
