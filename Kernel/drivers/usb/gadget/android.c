...
static int ums_adb_bind_config(struct usb_configuration *c)
{
	struct android_dev *dev = _android_dev;
	int ret;
...
static struct usb_configuration ums_adb_config  = {
	.label		= ANDROID_DEBUG_CONFIG_STRING,
	.bind		= ums_adb_bind_config,
...
static void enable_adb(struct android_dev *dev, int enable)
{
	...
	if (enable != dev->adb_enabled) {
		...
		if (enable)
		{
			ret = usb_change_config(dev->cdev, &ums_adb_config);
			if (ret) {
				printk("[%s] Fail to ums_adb_config()\n", __func__);
			}
		}
		else
		{
