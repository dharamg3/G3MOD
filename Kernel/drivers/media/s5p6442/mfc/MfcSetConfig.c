...
// Input arguments for IOCTL_MFC_SET_CONFIG
int MFC_GetConfigParams(MFCInstCtx *pMfcInst, MFC_ARGS *args)
{
	int	ret;
	...
	case MFC_GET_CONFIG_DEC_H264_CROPINFO:
		args->get_config.out_config_value[0] = pMfcInst->crop_value0;
		args->get_config.out_config_value[1] = pMfcInst->crop_value1;
		ret = MFCINST_RET_OK;
		break;
...
// Input arguments for IOCTL_MFC_SET_CONFIG
int MFC_SetConfigParams(MFCInstCtx *pMfcInst, MFC_ARGS *args)
{
	int	ret;
	...
	case MFC_SET_PADDING_SIZE:
		LOG_MSG(LOG_TRACE, "MFC_SetConfigParams", "MFC_SET_PADDING_SIZE : paddingsize(%d)\n", args->set_config.in_config_value[0]);		
