...
#endif
		   POWERCFG_BITSET_SKMODE_HIGH(&Si4709_dev.registers[POWERCFG]); /*VNVS:21-JAN-10---- Stop Seeking when upper or lower band limit is reached*/
...
int Si4709_dev_resume(void)
{
	...
	/*Reconfiguring the interrupt, as the Powermanager driver of the chip saves only 
	* GPJ2CON(which sets GPIO pin as interrupt) and doesnt save the states of registers 
	* GPJ2_INT_CON, GPJ2_INT_MASK, GPJ2_INT_PEND(which sets interrupt configuration like edge/level triggering, enable/disable)
	*/
    s3c_gpio_cfgpin(GPIO_FM_INT,S3C_GPIO_SFN(GPIO_FM_INT_STATE));
    s3c_gpio_setpull(GPIO_FM_INT,S3C_GPIO_PULL_UP);    
    set_irq_type(FM_IRQ_INT,IRQ_TYPE_EDGE_FALLING);
	
    enable_irq(FM_IRQ_INT); 
	...
	debug("Si4709_dev_resume call over");
...
static int powerup(void)
{
    ...
    if( (ret = i2c_write(POWERCFG)) < 0 )
    {
        debug("powerup->i2c_write failed");
        Si4709_dev.registers[POWERCFG] = powercfg;
    }
    else
    {
...
static int powerdown(void)
{
	...
	if(!(RADIO_POWERDOWN==Si4709_dev.state.power_state))
	{
		...
		/*VNVS: 2-APR'10---- SYSCONFIG1 write should happen before setting DISABLE bit*/
		if( (ret = i2c_write(SYSCONFIG1)) < 0 )
		{
			debug("powerdown->i2c_write 2 failed");
			Si4709_dev.registers[SYSCONFIG1] = sysconfig1;
			return ret;
		}
		...
		if( (ret = i2c_write( POWERCFG )) < 0 )
		{
			debug("powerdown->i2c_write 3 failed");
			Si4709_dev.registers[POWERCFG] = powercfg;			
		}
		else
		{
