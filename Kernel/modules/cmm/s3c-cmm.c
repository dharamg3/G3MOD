...
static long s3c_cmm_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	int                     ret;
	CODEC_MEM_CTX *         CodecMem;
	...
	switch (cmd) 
	{
		...
		case IOCTL_CODEC_CACHE_FLUSH:
			...
			dmac_unmap_area(start, codec_cache_flush_arg.size, PCI_DMA_BIDIRECTIONAL);
