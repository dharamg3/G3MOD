			    ...
			    unsigned int stride)
{
	...
	for (controller = 0; controller < S3C_DMA_CONTROLLERS; controller++) {
		...
		if(controller == 0) {
			// 091014 Univ6442 Sound (beta3) : dma_base = ioremap(S3C_PA_DMA, stride);
			dma_base = ioremap(S3C_PA_MDMA, stride);
			// 091014 Univ6442 Sound (beta3-for debugging only) :
			// printk("controller_0 : dma_base=%d(0x%x)", dma_base, dma_base);
